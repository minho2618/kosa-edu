...
SELECT truncate(avg(sal), 2) 평균, max(sal) 최고, min(sal) 최저, count(empno) 인원수
FROM emp
WHERE deptno=30;

-- 2) 각 부서별 급여의 평균(소수이하 반올림), 최고, 최저, 인원수를 구하여 출력하라.
SELECT deptno 부서번호, round(avg(sal), 0) 평균, max(sal) 최고, min(sal) 최저, count(empno) 인원수
FROM emp
GROUP BY deptno
ORDER BY 1;
...
SELECT job 업무명, count(empno) 인원수
FROM emp
GROUP BY job
...
SELECT floor(avg(sal)) "평균 월급", sum(sal) "전체 월급", max(sal) "최고 월급", min(sal) "최저 월급"
FROM emp
GROUP BY deptno
ORDER BY 1 DESC;
...
-- 단, 부서별 급여합계도 같이 출력하라.
SELECT deptno 부서번호, job 업무명, sum(sal) 급여합계
FROM emp
GROUP BY deptno, job WITH ROLLUP;


-- 7) 각 부서별 인원수를 조회하되 인원수가 5명 이상인 부서만 출력되도록 하시오.
SELECT deptno 부서번호, count(empno) 인원수
FROM emp
GROUP BY deptno
