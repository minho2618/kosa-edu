...
/* 
  ...
  이를 이름, 급여, 회비(소수점 세번째에서 반올림)를 출력하라.
*/
...
SELECT ename 이름, sal 급여, round(sal*0.05) 경조비
FROM emp
WHERE sal >= 2000;
 ...
 SELECT empno, ename, sal,  ceiling(comm*1.5) 보너스
 FROM emp
 ...
 */
 SELECT ename, sal, round(sal/12/5, 0) "시간당 임금"
 FROM emp
 WHERE deptno=20;
...
-- 5)  30번 부서의 연봉을 계산하여 이름, 부서번호, 급여, 연봉을 출력하라. 단, 연말에 급여의 170%를 보너스로 지급한다.
 SELECT ename, deptno, sal, (sal*12+ifnull(comm, 0)+sal*1.7) 연봉
 FROM emp
 WHERE deptno=30;
...
--  단, 커미션이 NULL인 사람은 제외한다.(커미션금액: sal*comm/100)
 SELECT ename, sal, sal*comm/100 comm, sal + comm total
 FROM emp
